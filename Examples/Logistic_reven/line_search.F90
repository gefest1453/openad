...
#undef VALUE_BASED
  ...
  subroutine LINE_SEARCH(NN,X0,GRAD0,F0,DIRECTION,EPS0,NITER,ctrlname,costname,if_restart,if_exceed,ncomp)
    ...
    logical if_restart,if_exceed,if_fail
#undef VALUE_BASED
#ifdef VALUE_BASED
    call LINE_SEARCH2(NN,X0,GRAD0,F0,DIRECTION,EPS0,NITER,ctrlname,costname,if_restart,if_exceed,ncomp)
#else
    call LINE_SEARCH1(NN,X0,GRAD0,F0,DIRECTION,EPS0,NITER,if_restart,if_exceed,ncomp,if_fail)
#endif    
...
300 if (fa<fb) then
  ...
  subroutine LINE_SEARCH1(NN,X0,GRAD0,F0,DIRECTION,EPS0,NITER,if_restart,if_exceed,ncomp,if_fail)
    use norma0
    implicit none
    integer ,intent(in)::nn
    INTEGER,INTENT(IN) ::NITER
    ...
    integer i,counter
    integer,intent(inout)::ncomp
    logical if_suff,if_restart,i_exist,ifsucc,if_exceed,if_fail
    character(len=9):: ctrlname,costname
111 FORMAT (A,E19.11E2,E19.11E2,E19.11E2)
112 FORMAT (i3,A,E19.11E2,E19.11E2,E19.11E2)
    gamma=1.0
...
40  if_suff=(((fa<0.9*f0).and.(wolfe_condition(f0,g0,fa,ffa,a))).or.((wolfe_condition(f0,g0,fb,ffb,b)).and.(fb<0.9*f0)))
    if_fail=(fa>f0).and.(fb>f0)
    do  while((.not. if_suff).and.(.not.if_fail))
       c=davidon(a,b,fa,fb,ffa,ffb)
       ...
       if_fail=(fa>f0).and.(fb>f0).and.(fc>f0)
       open(FILE="linesearch.sav",unit=171,action='write',status='replace',ERR=10)
       write(171,*)2
       close(171)
       call  dump_res(nn,a,grada,fa,'a')
...
IMPLICIT NONE   
 ...
 70   FORMAT(/'   I   NFN',4X,'FUNC',12X,'GNORM',12X,'STEPLENGTH'/)
